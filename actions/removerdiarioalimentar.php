...
/**
 * Action responsavél pela exclusão do registro de diario de alimentos
 * 
 * @package Default
...
$v_app_retorno  = [];
$v_alimento_id  = ((isset($_POST['diarioalimentar_excluir_id'])) ? $_POST['diarioalimentar_excluir_id'] : '');
$fsessao        = new Session();
$v_session_code = ((isset($_SESSION['code'])) ? $_SESSION['code'] : ((isset($_COOKIE['code'])) ? $_COOKIE['code'] : ''));
if ($v_session_code !== '' && $v_alimento_id !== '') {
    $fdiarioalimento = new Diarioalimentar();
    $v_alimento      = $fdiarioalimento->desativarDiarioalimentar($v_alimento_id, '0');
